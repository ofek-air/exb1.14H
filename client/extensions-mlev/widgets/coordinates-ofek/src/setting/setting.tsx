/** @jsx jsx */
import { React, jsx, Immutable, defaultMessages as jimucoreMessages, urlUtils, moduleLoader, hooks, FormattedMessage } from 'jimu-core'//_ml_ added FormattedMessage
import { type AllWidgetSettingProps, builderAppSync } from 'jimu-for-builder'
import { Button, defaultMessages as jimuiMessages, Icon, Label, NumericInput, Radio, Switch, Tooltip, TextArea } from 'jimu-ui'//_ml_ added item TextArea
...
//import type * as JimuCoreWkid from 'jimu-core/wkid'//_ml_ copied from 1.17
...
const Setting = (props: SettingProps): React.ReactElement => {
  const {
    ...
    config: propConfig,
    useMapWidgetIds//,
    //label //_ml_ _gaya_ added the Widget label for allowance alerts, but commented since no allowance check on setting
  } = props
  const {
    showBaseMapInfo, showScale, showZoom, showTilt, showRotation, token, ofek3dAppUrl, ofekCfgObliqueAppUrl,//_ml_ added line
    coordinateSystem, showSeparators, coordinateDecimal, altitudeDecimal, displayOrder, widgetStyle, mapInfo, mapInfo2 } = propConfig
  ...
  //_ml_ added block top - see below in HTML
  const showBaseMapInfoLabel = translate('showBaseMapInfo')
  const showScaleLabel = translate('showScale')
  const showZoomLabel = translate('showZoom')
  const showTiltLabel = translate('showTilt')
  const showRotationLabel = translate('showRotation')
  const ofekCfgObliqueProjectUrlLabel = translate('ofekCfgObliqueAppUrl')
  const ofek3dAppUrlLabel = translate('ofek3dAppUrl')
  const tokenLabel = translate('token')
      ...
      {useMap &&
        <Fragment>
          ...
          >
            ...
            {/*_ml_ added block top - additional labels */}
            <SettingRow label={showBaseMapInfoLabel}>
              <Switch
                className='can-x-switch'
                checked={showBaseMapInfo}
                data-key='showBaseMapInfo'
                onChange={evt => {
                  onPropertyChange('showBaseMapInfo', evt.target.checked)
                }}
                aria-label={showBaseMapInfoLabel}
              />
            </SettingRow>
            <SettingRow label={showScaleLabel}>
              <Switch
                className='can-x-switch'
                checked={showScale}
                data-key='showScale'
                onChange={evt => {
                  onPropertyChange('showScale', evt.target.checked)
                }}
                aria-label={showScaleLabel}
              />
            </SettingRow>
            <SettingRow label={showZoomLabel}>
              <Switch
                className='can-x-switch'
                checked={showZoom}
                data-key='showZoom'
                onChange={evt => {
                  onPropertyChange('showZoom', evt.target.checked)
                }}
                aria-label={showZoomLabel}
              />
            </SettingRow>
            <SettingRow label={showTiltLabel}>
              <Switch
                className='can-x-switch'
                checked={showTilt}
                data-key='showTilt'
                onChange={evt => {
                  onPropertyChange('showTilt', evt.target.checked)
                }}
                aria-label={showTiltLabel}
              />
            </SettingRow>
            <SettingRow label={showRotationLabel}>
              <Switch
                className='can-x-switch'
                checked={showRotation}
                data-key='showRotation'
                onChange={evt => {
                  onPropertyChange('showRotation', evt.target.checked)
                }}
                aria-label={showRotationLabel}
              />
            </SettingRow>
            {/*_ml_ added block end*/}
            {/*_ml_ added block top - modal box*/}
            <SettingRow>
              <div style={{ wrap: 'wrap !important' }} className='w-100'>
                <label style={{ display: 'contents' }}>
                  <FormattedMessage id='token' defaultMessage={tokenLabel} />
                </label>
                <TextArea
                  className='w-100'
                  height={'40'}
                  ...
                  defaultValue={token ?? ''}
                  onBlur={evt => { onPropertyChange('token', evt.target.value) }}
                //onChange={evt => { onPropertyChange('myMapName', evt.target.value) }}
                />
              </div>
            </SettingRow>
            <SettingRow>
              <div style={{ wrap: 'wrap !important' }} className='w-100'>
                <label style={{ display: 'contents' }}>
                  <FormattedMessage id='ofek3dAppUrl' defaultMessage={ofek3dAppUrlLabel} />
                </label>
                <TextArea
                  className='w-100'
                  height={'70'}
                  aria-label={ofek3dAppUrlLabel}
                  placeholder={ofek3dAppUrlLabel}
                  defaultValue={ofek3dAppUrl ?? ''}
                  onBlur={evt => { onPropertyChange('ofek3dAppUrl', evt.target.value) }}
                //onChange={evt => { onPropertyChange('ofek3dAppUrl', evt.target.value) }}
                />
              </div>
            </SettingRow>
            <SettingRow>
              <div style={{ wrap: 'wrap !important' }} className='w-100'>
                <label style={{ display: 'contents' }}>
                  <FormattedMessage id='ofekCfgObliqueAppUrl' defaultMessage={ofekCfgObliqueProjectUrlLabel} />
                </label>
                <TextArea
                  className='w-100'
                  height={90}
                  aria-label={ofekCfgObliqueProjectUrlLabel}
                  placeholder={ofekCfgObliqueProjectUrlLabel}
                  defaultValue={ofekCfgObliqueAppUrl ?? ''}
                  onBlur={evt => { onPropertyChange('ofekCfgObliqueAppUrl', evt.target.value) }}//same as onChange
                //onChange={evt => { onPropertyChange('ofekCfgObliqueAppUrl', evt.target.value) }}
                //_ml_ added block end
                />
              </div>
            </SettingRow>
