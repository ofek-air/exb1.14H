...
import * as ofekUtils from '/ofekUtils'//_ml_ added import co_is__alone_not
...
const Widget = (props: AllWidgetProps<IMConfig>): React.ReactElement => {
  ...
  const { coordinateSystem, coordinateDecimal, altitudeDecimal, showSeparators, displayOrder, widgetStyle, mapInfo, mapInfo2,
    showBaseMapInfo//_ml_ added item
  } = config
  ...
  // _gali_ STEP_1_MINIMAL: Ref to store the mapWidgetId for this instance
  //const mapWidgetIdRef = useRef<string | null>(null);//_gali_ added
  ...
  const openNewTab = (nawTabName) => {//_ml_ route_newTab-00 _bmk_
    ...
    let widget = (_app.newTab?.name) ? _app.newTab.name : "coordinates";//@openNewTab co_is__alone_not
  ...
  const clearNewTabWidgets = () => {//_ml_ added func _bmk_
    //const _app = window._app_;
    if (_app.newTab?.name) {//@clearNewTabWidgets co_is__alone_not
      ofekUtils.setImageOfNewTabWidget(_app.newTab?.name);
    }
    _app.newTab = null; //@clearNewTabWidgets co_is__alone_not
  ...
  const toggleIsOpenModalCoord = () => {//_ml_ route_newTab-04 _bmk_
    ...
    if (isOpenModalCoord) {
      ...
      if (_app.newTab) {
        if (_app.newTab?.name) {// co_is__alone_not
          ofekUtils.setImageOfNewTabWidget(_app.newTab?.name);//@clearNewTabWidgets
        }
        _app.newTab = null; //@toggleIsOpenModalCoord co_is__alone_not
      }
    } else {//co_is__alone_not
  ...
  const ofekCfgObliqueAppUrl = _app.obliqueApp.ofekCfgObliqueAppUrl//_ml_ added const
  ofekUtils.__geoXfrm_setup(2039, 108021)//_ml_ added

  useEffect(() => {
    //// _gali_ added block top - Call initGlobalApp and store mapWidgetId
    //mapWidgetIdRef.current = ofekUtils.initGlobalApp(props.id, props.config.token || null, 'coordinates-ofek');
    //const _app = window._app_;
    //const mapContext = _app?.maps?.[mapWidgetIdRef.current || 'noMap'];
    //
    //// _gali_ Allowance check
    //if (!mapContext?.isWidgetAllowed) {
    //  const widgetElement = document.getElementById(`widget-${props.id}`);
    //  if (widgetElement) {
    //    //widgetElement.style.display = 'none';
    //  }
    //  if (_app?.isDbgLog) console.log(`coordinates-ofek (${props.id}) NOT ALLOWED and hidden during mount. SKIPPED`);
    //  //return;
    //}
    //if (_app?.isDbgLog) console.log(`coordinates-ofek (${props.id}) IS ALLOWED.`);
    ////_gali_ added blcok end
    //
    //// _gali_ Store map-specific info if this is the coordinates widget and we have a valid map ID
    //if (mapWidgetIdRef.current && mapContext) {
    //  mapContext.coordinatesWidgetId = props.id; // Store this widget's ID in its map's context
    //  // _gali_ Populate obliqueApp and ofek3dAppUrl from widget config into map context
    //  mapContext.obliqueApp = mapContext.obliqueApp || { name: null, ofekCfgObliqueAppUrl: "" };
    //  mapContext.obliqueApp.ofekCfgObliqueAppUrl = ofekUtils.isBlank(props.config.ofekCfgObliqueAppUrl) ? "" : props.config.ofekCfgObliqueAppUrl;
    //  // _gali_ Determine ObliqueApp.name based on URL (as in arcgis-map-ofek)
    //  let newTabUrl = mapContext.obliqueApp.ofekCfgObliqueAppUrl.toLowerCase();
    //  if (newTabUrl.includes("publication")) {
    //    mapContext.obliqueApp.name = ofekUtils.ObliqueApp.Orbit;
    //  } else if (newTabUrl.includes("oblivisionjs")) {
    //    mapContext.obliqueApp.name = ofekUtils.ObliqueApp.Idan;
    //  } else {
    //    mapContext.obliqueApp.name = null;
    //  }
    //  mapContext.ofek3dAppUrl = props.config.ofek3dAppUrl; // Assuming ofek3dAppUrl is in props.config
    //  if (_app?.isDbgLog) console.log(`coordinates-ofek: Map context for ${mapWidgetIdRef.current} updated with its info:`, mapContext);
    //}

    //_ml_ added block top fitContent
    ...
    //_ml_ added block end fitContent
    return () => {
  ...
  hooks.useUpdateEffect(() => {
    // _gali_ This effect manages mouse move and click listeners based on locateActive.
    // _gali_ For Step 1, inter-widget state (_app.maps[mapId].isNewTabWaitingForClick) is NOT considered here.
  ...
  useEffect(() => {
    if (!isControlMapWidget) {
      resetAllGeoInfo()
      if (graphicsLayer.current && markerGraphic.current) // _gali_ Check both
        graphicsLayer.current.remove(markerGraphic.current)
      markerGraphic.current = null
    }
  }, [isControlMapWidget])
  ...
  const onActiveViewChange = (jimuMapView: JimuMapView) => {
    ...
    ofekUtils.rdwt(widgetName, "currentJimuMapView", "wt", jimuMapView)//_ml_ added
  ...
  const onMouseMove = async (point, threeDPoint?) => {//_ml_ _bmk_ cc in mark mode we dont come here
    ...
    //_ml_ added block top co_is__alone_not
    ...
    //_ml_ added block end co_is__alone_not
  }
  ...
  //const _setEyeInfo = () => {//_ml_ func replaced
  //  const view = currentJimuMapView?.view
  ...
  const _setEyeInfo = () => {//_ml_ modified func
    ...
    if (!view || !view?.camera || !view?.camera?.position) {
      ...
      setEyeNum(null)
      setEyeUnit('')
      setEyeInfoAlt('')//_ml_ added line
      setEyeInfoTilt('')//_ml_ added line
      setScaleInfo('')//_ml_ added line
      setZoomInfo('')//_ml_ added line
      setRotationInfo('')//_ml_ added line
      return
    }
    //_ml_ added block top
    let scale = Math.round(view.scale * 1) / 1
  ...
  const onLocateClick = async () => {//_ml_   _bmk_
    ...
    if (locateActive && _app.newTab) {//@onLocateClick route_newTab-09 shut-off all 3 newTab image-widgets
      ...
      _app.newTab = null; //@onLocateClick co_is__alone_not @onLocateClick
    }
    //_ml_ added block end
